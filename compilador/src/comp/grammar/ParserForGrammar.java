...
import comp.Token;
import comp.TokenType;

//https://www.regextester.com/15

public class ParserForGrammar {
	
	Scanner scan;
	int count = 0;
	Stack<Token> tokens;
	
	public ParserForGrammar(String args) {
		try {
			scan = new Scanner(new File(args));
		}catch (FileNotFoundException e) {
			System.err.println("Arquivo '"+ args +"' nao encontrado.");
			System.exit(-1);
		}
	}
	
	public Stack<Token> nextToken() {
		//Token retorno = null;
		if(scan.hasNextLine()) {
			...
			while(i<linha.length()){
				...
				if (i < linha.length()) {
					...
					if(lexema.equals(TokenType.DOUBLE_QUOTES.toString())){
						do{
							lexema += linha.charAt(i);
						...
						if(lexema.charAt(lexema.length()-1) == '"') {
							tokens.push( dAT(LIT_STRING, lexema, count, i-lexema.length()) );		
							lexema = "";
						} else{
							tokens.push( dAT(RD_ERROR, lexema, count, i-lexema.length()) );		
							lexema = "";
						}
					}
					...
					// capture identifiers
						case ',': case '(': case '=': case ':': case '{': 
							if (!contains(lexema) && !Character.isDigit(lexema.charAt(0))) { 
								tokens.push( dAT(IDENTIFIER,	lexema, count, i-lexema.length()) );		
								lexema = "";
					...
					// get float numbers
							...
							if( lexema.matches("\\d+") ) {
								if(Character.toString(linha.charAt(i+1)).matches("\\d")){
									...
									if(Pattern.matches("\\.\\d+((E|e)(\\+|-)?\\d+)?", fraction)){
										tokens.push( dAT(LIT_REAL, lexema, count, i-lexema.length()) );										
										lexema = "";
										//System.out.print(tokens.getFirst().type.name() +" ");
									}
								}
						...
						case '>': case '<': case ' ': 
							if( lexema.matches("\\d+")) {
								tokens.push( dAT(LIT_INT, lexema, count, i-lexema.length()) );		
								lexema = "";
				...
				if (b) {
					tokens.push( dAT(getTokenType(lexema), lexema, count, i-lexema.length()) );		
